/*
  ...
  1. Changes
    - Update kudos_stats_weekly view to award 1 point per kudos given
    - Update kudos_stats_monthly view to award 1 point per kudos given
    - Remove points for receiving kudos
*/

-- Drop existing views
DROP VIEW IF EXISTS kudos_stats_weekly;
DROP VIEW IF EXISTS kudos_stats_monthly;

-- Recreate weekly stats view with new point system
CREATE
OR REPLACE VIEW kudos_stats_weekly AS
...
SELECT u.id                                as user_id,
       ...
       COALESCE(SUM(kc.kudos_given), 0)    as total_points,
...
-- Recreate monthly stats view with new point system
CREATE
OR REPLACE VIEW kudos_stats_monthly AS
WITH kudos_counts AS (
  -- Count kudos given
  SELECT
    giver_id as user_id,
